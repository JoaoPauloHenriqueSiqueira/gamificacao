...
class AccountUserValidator extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        if ($admin && !$email) {
            $valid['email'] = Rule::unique('users')
                ->ignore($this->request->get('id'))->where(function ($query) use ($companyId, $userId) {
                    return $query->where('company_id', $companyId)->where('id', $userId);
                });
        }
