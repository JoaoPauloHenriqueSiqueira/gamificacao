    ...
    public function update($request)
    {
        if ($request->validated()) {
            ...
            if($card->cardNumber == $request->cardNumber){
                return response('Esse é o seu cartão cadastrado atualmente, por favor cadastre outro cartão', 422);
            }

            $response = $this->repository->updateOrCreate(["id" => $card->id], $request->except('brand','cvv', 'expirationMonth', 'expirationYear'));
    ...
    public function save($request)
    {
        try {
            if ($request->validated()) {
                ...
                if (!$card) {
                    $response = $this->repository->create($request->except('brand','cvv', 'expirationMonth', 'expirationYear'));
                }
